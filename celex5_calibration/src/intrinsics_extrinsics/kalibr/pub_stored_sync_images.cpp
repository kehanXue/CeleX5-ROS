          ...
          publish_rate_(publish_rate)
{
    ...
    p_ddyn_rec_->registerVariable<bool>("is_publish_enable", &is_publish_enable_, "Publish enable.", false, true);
    ...
    p_main_thread_ = std::make_shared<std::thread>([&]()
                                                   {
                                                       ...
                                                       p_ddyn_rec_->registerVariable<int>("publish_rate", publish_rate_,
                                                                                          [&loop_rate](int new_fps)
                                                                                          {
                                                                                              loop_rate = ros::Rate(
                                                                                                      new_fps);
                                                                                          }, "Publish rate. Unit hz.",
                                                                                          0, 200);

                                                       if (image1_filenames_.size() != image1_filenames_.size())
                                                       {
                                                           ROS_ERROR(
                                                                   "The each number of sync images set doesn't equal!");
                                                           return;
                                                       }
                                                       ...
                                                       auto read_file_and_publish =
                                                               [&index, &now_stamp](const ros::Publisher &pub,
                                                                                    const std::vector<cv::String> &image_filenames)
                                                               {
                                                                   cv::Mat image = cv::imread(
                                                                           image_filenames.at(index));
